...
`MODULE_ID`, `SETTINGS` (key names), `registerSettings` — 4 world-scope settings:
...
default `false`) and `defaultGridShape` (String, `"square"`/`"hex"` with a `choices` dropdown,
default `"square"` — hex is pointy-top, odd-row offset only, docs/DECISIONS.md "Hex grid
support") both read by `generation/geography.mjs`'s `generateGeography` (passed into
`rollGeographyBatch`/`rollGeographyStep` and `placeTerrainRolls`/`createGeographyScene`
respectively — `defaultGridShape` only on a region's *first* Geography run, since
`region.geography.gridShape` is stored from then on and a re-run reuses it), `generateNames`
(Boolean, default `true` — read by `generation/settlements.mjs`, gates `generation/names.mjs`'s
calls). Every setting is read at its one Foundry-side call site and threaded down as a plain
parameter into the pure roll functions below it, so those stay unit-testable without a
`game.settings` stub — same pattern Hazards' GM-chosen lair style uses for
`generateHazards(region, style)`.
...
|------|---------|-------------------|
| `region.mjs` | `REGION_PHASES`, `createRegion`, `runPhase`, `isPhaseDone` | phase registry + region data shape (`geography.gridShape`, `"square"`/`"hex"`, defaults `"square"` as an unset placeholder — see `generateGeography`); `isPhaseDone` centralizes each phase's "has this produced anything yet" check for the wizard's checklist |
| `geography.mjs` | `rollGeographyStep`, `rollSpecialFeature`, `rollGeographyBatch`, `generateGeography` | GEOGRAPHY PROCESS rolls (Tables 1-1, 1-2), redesigned to run as a single batch (docs/DECISIONS.md "Geography (redesign)"). `rollGeographyBatch` loops `rollGeographyStep` up front, stopping once cumulative *terrain* `size` alone meets the map's capacity (rivers/specials excluded from that budget — they never need their own space). `rollGeographyStep` optionally rerolls a banned Table 1-1 total (the "Ban Large Geography Regions" setting) before proceeding — 81-99 on a map under 500 squares, 91-99 on 500+ — capped at 20 attempts as a safety net; the discarded rolls never bump the running bonus. `generateGeography` is the one-shot `runPhase` orchestrator: rolls the batch, places terrain (`geography-terrain.mjs`), then Isolated Mountain rolls specifically (`geography-features.mjs`'s `placeIsolatedMountains`, *before* rivers so they're honored as high ground rivers won't flow uphill onto), then rivers (`geography-rivers.mjs`), then every remaining Special Feature (`geography-features.mjs`'s `placeSpecialFeatures`, Isolated Mountain rolls filtered out since they're already placed), then computes any hex border fillers (`geography-border.mjs`'s `hexBorderFillers`, a no-op on a square grid), then paints/journals/chats it (the Foundry-effects files below) |
| `geography-grid.mjs` | `createPlacementGrid`, `isBorderCell`, `allCells`, `freeCells`, `freeBorderCells`, `cellsOfRegion`, `cellsOfType`, `neighborsOf`, `cellsAdjacentTo`, `nearestDistance`, `distanceToBorder`, `pickRandomCell`, `claimBlobFromSeed`, `cellDistance`, `offsetToCube`, `neighborsOfCube` | Generic grid/blob primitives shared by the placement modules below — no per-terrain-type rules live here. `createPlacementGrid`'s `type` ("square" default, or "hex" — pointy-top, odd-row offset only, docs/DECISIONS.md "Hex grid support") is the only thing any of this dispatches on; every cell everywhere still addresses by the same `{x,y}` offset coordinates. `neighborsOf` is 8-directional for square, 6-directional (via internal offset↔cube conversion) for hex. `cellDistance` is the grid-type-aware distance every placement rule (Mountains-farthest-from-Swamp, river bias, Cave radius, blob fill order) goes through instead of hand-rolled `Math.hypot` — Euclidean for square, cube tile-step distance for hex. No Foundry dependency, fully unit-tested |
...
| `geography-features.mjs` | `placeIsolatedMountains`, `placeSpecialFeatures` | Places every Table 1-2 special feature roll, each per its own terrain preference/avoidance rule (see docs/DECISIONS.md's full per-feature table) — always a **simple overwrite** of whatever terrain was there, never relocating it, and never on a map-edge cell (`interiorCells`) unless the feature is anchored to a river's path (Waterfall/Whirlpool, or Geyser reusing an existing river) — rivers can legitimately reach the border. **`placeIsolatedMountains` runs separately, before rivers** — the only Special Feature placed ahead of `placeRivers`, so its cell is honored as Mountains-tier elevation (`ELEVATION_TIER["Isolated Mountain"]`) a river can't flow uphill onto; `placeSpecialFeatures` expects Isolated Mountain rolls already filtered out of what it's given. Geyser/Waterfall/Whirlpool can each trigger "generate a river first" when none exist yet (routed around every cell — and diagonal crossing — already-placed rivers used, via `usedRiverCells`/`usedRiverDiagonals`); `realPathCells` strips a river's trailing synthetic border-exit/Swamp-touch point before either ever tries to place something "on" it. Cliff traces the *connected* shared boundary chain between its two picked regions, corner-by-corner (`sharedEdgeCorners` + `walkBoundaryChain`), stopping wherever the chain actually breaks (a branch point, or a third blob wedging in) rather than jumping to a disconnected stretch of the same two regions elsewhere on the map — a live-tested fix. `sharedEdgeCorners` dispatches on grid type: square returns exact integer grid-line corners; hex returns each corner as the 3 cube coordinates of the (up to 3) hexes meeting there (`neighborsOfCube(A) ∩ neighborsOfCube(B)`, docs/DECISIONS.md "Hex grid support") — every corner (either type) carries a `.key` string so `walkBoundaryChain` compares by identity alone, unchanged for either grid type. Caves' Scene label names the terrain each entrance replaced ("Cave entrance in Grassy Hills," `caveEntranceLabel`) via a `label` field distinct from `terrain`. Pure, unit-tested |
| `geography-border.mjs` | `hexBorderFillers` | Decides which of a hex Scene's blank boundary slivers exist (column slivers left/right, notch slivers top/bottom — docs/DECISIONS.md "Hex grid support") and what color/opacity each gets — majority of its real bordering cell(s)' terrain (randomly tie-broken), that cell's own vegetation `fillAlpha`, or the river's color/opacity if a bordering cell is a genuine river exit. Returns `[]` entirely for a square grid. Pure, unit-tested; the actual pixel geometry is `geography-scene.mjs`'s `paintHexBorderFillers` |
| `geography-scene.mjs` | `createGeographyScene`, `paintGrid`, `paintHexBorderFillers`, `paintRivers`, `paintCliffs` | Foundry `Scene`/`Drawing` creation — zero scene padding, Global Illumination enabled, reuses an existing scene (clearing its Drawings) on a re-run instead of creating a second one; `gridShape` param sets the Scene's own `grid.type` (`CONST.GRID_TYPES.HEXODDR` or `SQUARE`). Every painter reads the grid type back off the live `scene.grid` instance (`scene.grid.isHexagonal`) rather than being told separately. `paintGrid` batches one Drawing per grid cell into a single `createEmbeddedDocuments` call (fill color from `FEATURE_COLORS`, `fillAlpha` from `VEGETATION_OPACITY`, label from `cell.label` if set else vegetation+terrain) — a rectangle for square, a hexagon polygon Drawing (`scene.grid.getVertices`) for hex. `paintHexBorderFillers` turns `geography-border.mjs`'s `hexBorderFillers` data into real pixel geometry — a column sliver clips a virtual off-grid hex to the Scene's own bounds (Sutherland-Hodgman half-plane clip), a notch sliver is a triangle built from two real adjacent cells' own vertex lists (each hex's extreme peak/trough vertex plus their shared edge's near endpoint). `paintRivers`/`paintCliffs` paint one freehand polygon Drawing per path, each with its own pixel conversion — rivers through cell *centers* (river paths are cell coordinates, `strokeWidth: 8`, `bezierFactor: 1`; hex uses `hexRiverPoint`, lerping between real hex centers for a fractional border/Swamp-touch point), cliffs through raw corner points *as-is* for square (cliff paths are already exact grid-line coordinates, `strokeWidth: 10`, unsmoothed) or the average of a hex corner's (up to 3) `cubeToPoint` centers for hex (docs/DECISIONS.md "Hex grid support") — isolated from the pure logic above so that stays unit-testable |
...
| `ruins-scene.mjs` | `createRuinsJournal`, `placeRuinNotes` | Creates/appends to "`<Map Name>` - Ancient Ruins" (one page per ruin) and pins a scene `Note` per ruin, deep-linked to its page and centered on its cell — real hex center via `scene.grid.getCenterPoint` on a hex Scene, same `scene.grid.isHexagonal` branch as `geography-scene.mjs`; `ruins.mjs`'s own cell picking stays grid-shape-agnostic — isolated from the pure logic above so that stays unit-testable |
